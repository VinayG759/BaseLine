...
def test_seed_creates_the_demo_people_and_two_reports(configured_aws):
    seed_demo.seed(aws_services())

    services = aws_services()
    people = {p.display_name: p for p in services.list_people()}
...
def test_seeding_twice_does_not_duplicate_anything(configured_aws):
    ...
    services = aws_services()
    assert len(services.list_people()) == 2
    sunita = next(p for p in services.list_people() if p.name == "Sunita Rao")
